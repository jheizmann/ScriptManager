...
function smfAddHTMLHeader(& $out) {
	...
	foreach($smgJSLibs as $lib_id) {
		...
		switch($lib_id) {
			case 'prototype':
				$out->addScript("<script type=\"text/javascript\" src=\"". "$smgSMPath/scripts/prototype.js$scmgStyleVersion\" id=\"Prototype_script_inclusion\"></script>");
				break;
			case 'jquery':
			...
			case 'qtip':
					...
					$out->addScript("<script type=\"text/javascript\" src=\"". "$smgSMPath/scripts/qTip/jquery.qtip-1.0.0-rc3.min.js$scmgStyleVersion\"></script>");
                else
					$out->addScript("<script type=\"text/javascript\" src=\"". "$smgSMPath/scripts/qTip/jquery.qtip-1.0.0-rc3.js$scmgStyleVersion\"></script>");
				break;
			case 'json':
                if (isset($smwgDeployVersion) && $smwgDeployVersion !== false)
				...
				break;
			case 'fancybox':
				$out->addScript("<script type=\"text/javascript\" src=\"". "$smgSMPath/scripts/fancybox/jquery.fancybox-1.3.1.js$scmgStyleVersion\"></script>");
				$out->addStyle($smgSMPath."/scripts/fancybox/jquery.fancybox-1.3.1.css$scmgStyleVersion", 'screen, projection');
				break;
			case 'ext':
